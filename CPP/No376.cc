...
public:
    int wiggleMaxLength(vector<int> &nums) {
        int size = nums.size();
        if (size < 2) return size;  // 返回特例，小于等于 1 的时候，直接返回数据长度
        int ans = 1, pre = 0;
        for (int i = 1; i < nums.size(); ++i) { // 开始检查相邻元素的差值
            int curr = nums[i] - nums[i - 1];   // 记录当前元素 i 和前一个元素 i - 1 的差值
            // 当前差值和前一个差值进行对比, 如果当前的大于 0 ,而且前一个是小于等于 0, 那么就说明波动了一下, 记下来
            if ((curr > 0 && pre <= 0) || (curr < 0 && pre >= 0)) {
                ++ans;
                pre = curr; // 将当前差值记作前次差值
            }
        }
        return ans;
    }
